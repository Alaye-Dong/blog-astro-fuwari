...
function validateExtensionEntry(setting, plugin) {
	...
	if (setting.extensionOverride !== void 0) {
		...
		if (extOverride.some((ext) => ext.extension === "")) {
			wrongIndex.push({
				type: "empty",
				index: extOverride.findIndex((ext) => ext.extension === ""),
			});
		}
		...
		if (duplicate.length > 0) {
			duplicate.forEach((dupli) => {
				wrongIndex.push({
					type: "duplicate",
					index: extOverride.findIndex((ext) => dupli === ext.extension),
				});
			});
		}
		...
		if (markdown.length > 0) {
			wrongIndex.push({
				type: "md",
				index: extOverride.findIndex((ext) => ext.extension === "md"),
			});
		}
		...
		if (canvas.length > 0) {
			wrongIndex.push({
				type: "canvas",
				index: extOverride.findIndex((ext) => ext.extension === "canvas"),
			});
		}
		const excludedFromSettings = plugin.excludeExtensionPattern.split("|");
		const excluded = extOverride.filter((ext) =>
			excludedFromSettings.includes(ext.extension),
		);
		if (excluded.length > 0) {
			wrongIndex.push({
				type: "excluded",
				index: extOverride.findIndex((ext) =>
					excludedFromSettings.includes(ext.extension),
				),
			});
		}
...
function generateErrorExtensionMessage(type) {
	if (type === "canvas") {
		new import_obsidian.Notice(
			"Canvas is not supported as an extension override.",
		);
	} else if (type === "md") {
		new import_obsidian.Notice(
			"Markdown is not supported as an extension override.",
		);
	} else if (type === "empty") {
		...
		new import_obsidian.Notice(
			"Extension override cannot be an excluded extension.",
		);
	}
}
...
function getExtensionOverrideSetting(extension, settings) {
	if (
		settings.extensionOverride === void 0 ||
		settings.extensionOverride.length === 0
	) {
...
var OverrideExtensionModal = class extends import_obsidian2.Modal {
	...
	displaySw(cont) {
		cont.findAll(".setting-item").forEach((el) => {
			var _a;
			if (
				(_a = el.getAttr("class")) == null
					? void 0
					: _a.includes("override_root_folder_set")
			) {
...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
	...
	display() {
		...
		new import_obsidian3.Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						debugLog(
							"setting - automatically rename attachment folder:" + value,
						);
		...
		new import_obsidian3.Setting(containerEl)
			...
			.setDesc(
				"Using the extension override if you want to autorename the attachment with a specific extension (e.g. pdf or zip).",
			)
			.addButton((btn) => {
					...
					.addButton((btn) => {
						btn
							...
							.onClick(async () => {
								...
								const index =
									(_b =
										(_a = this.plugin.settings.attachPath.extensionOverride) ==
										null
											? void 0
											: _a.indexOf(ext)) != null
										? _b
										: -1;
								(_c = this.plugin.settings.attachPath.extensionOverride) == null
									? void 0
									: _c.splice(index, 1);
								await this.plugin.saveSettings();
								this.display();
							...
							.onClick(async () => {
								...
								if (wrongIndex.length > 0) {
									for (const i of wrongIndex) {
										...
										const wrongSetting = containerEl.getElementsByClassName(
											"override_extension_set",
										)[resIndex];
				...
				new import_obsidian4.Setting(contentEl)
					...
					.addButton((btn) => {
						btn.setIcon("trash").onClick(async () => {
							...
							const index =
								(_b =
									(_a = this.setting.extensionOverride) == null
										? void 0
										: _a.indexOf(ext)) != null
									? _b
									: -1;
							(_c = this.setting.extensionOverride) == null
								? void 0
								: _c.splice(index, 1);
							this.onOpen();
						});
			...
			.addButton((btn) => {
				btn.setButtonText("Reset").onClick(async () => {
					...
					new import_obsidian4.Notice(
						`Reset attachment setting of ${this.file.path}`,
					);
					this.close();
				});
			})
			.addButton((btn) =>
				btn
					...
					.onClick(async () => {
						...
						await this.plugin.saveSettings();
						debugLog(
							"override - overriding settings:",
							this.file.path,
							this.setting,
						);
						new import_obsidian4.Notice(
							`Overridden attachment setting of ${this.file.path}`,
						);
						this.close();
					}),
			);
...
function getRenameOverrideSetting(settings, file, oldPath) {
	...
	const { settingPath: op, setting: os } = getOverrideSetting(
		settings,
		file,
		oldPath,
	);
	...
	if (wikiMatches) {
		...
		for (const wikiMatch of wikiMatches) {
			if (matchIsWikiTransclusion(wikiMatch)) {
				...
				const file = app2.metadataCache.getFirstLinkpathDest(
					fileName,
					mdFile.path,
				);
				if (fileName !== "") {
...
var Metadata = class {
	...
	async getAttachFileName(
		...
		if (this.attachmentFile !== void 0) {
			...
			const { extSetting } = getExtensionOverrideSetting(
				this.attachmentFile.extension,
				setting,
			);
...
function containOriginalNameVariable(setting, ext) {
	...
	if (
		(extSetting !== void 0 &&
			extSetting.attachFormat.contains(SETTINGS_VARIABLES_ORIGINALNAME)) ||
...
function loadOriginalName(settings, setting, ext, md5) {
	if (containOriginalNameVariable(setting, ext)) {
		...
		const last = settings.originalNameStorage
			.reverse()
			.find((data) => data.md5 === md5);
...
var ArrangeHandler = class {
	...
	async rearrangeAttachment(type, file, oldPath) {
		...
		const attachments = await this.getAttachmentsInVault(
			this.settings,
			type,
			file,
			oldPath,
		...
		debugLog(
			"rearrangeAttachment - attachments:",
		...
		for (const obNote of Object.keys(attachments)) {
			...
			if (
				!(innerFile instanceof import_obsidian8.TFile) ||
				isAttachment(this.settings, innerFile)
			) {
				debugLog(
					`rearrangeAttachment - ${obNote} not exists or is attachment, skipped`,
				);
				continue;
			}
			...
			const attachPath = md.getAttachmentPath(
				setting,
			...
			if (!(await this.app.vault.adapter.exists(attachPath, true))) {
				if (
					oldPath != void 0 &&
					(await this.app.vault.adapter.exists(attachPath, false))
				) {
					const mdOld = getMetadata(oldPath);
					const attachPathOld = mdOld.getAttachmentPath(
						setting,
						this.settings.dateFormat,
					);
			...
			for (let link of attachments[obNote]) {
				...
				if (
					linkFile === null ||
					!(linkFile instanceof import_obsidian8.TFile)
				) {
				...
				const originalName = loadOriginalName(
					this.settings,
					setting,
					linkFile.extension,
					md5,
				...
				if (containOriginalNameVariable(setting, linkFile.extension)) {
					attachName = await metadata.getAttachFileName(
						setting,
						this.settings.dateFormat,
						(_a = originalName == null ? void 0 : originalName.n) != null
							? _a
							: "",
				...
				if (
					attachPath == path.dirname(link) &&
					attachName === path.basename(link, path.extname(link))
				) {
					continue;
				}
				...
				if (
					attachPathFile === null ||
					!(attachPathFile instanceof import_obsidian8.TFolder)
				) {
					debugLog(`${attachPath} not exists, skipped`);
					continue;
				}
				const { name } = await deduplicateNewName(
	...
	async getAttachmentsInVault(settings, type, file, oldPath) {
		...
		attachmentsRecord = await this.getAttachmentsInVaultByLinks(
			settings,
			type,
			file,
			oldPath,
	...
	async getAttachmentsInVaultByLinks(settings, type, file, oldPath) {
		...
		if (type == 1 /* LINKS */) {
			...
			if (file2) {
				if (
					(file2.parent && isExcluded(file2.parent.path, this.settings)) ||
					isAttachment(this.settings, file2)
				) {
					allFiles = [];
					...
					if (this.app.metadataCache.resolvedLinks[file2.path]) {
						resolvedLinks[file2.path] =
							this.app.metadataCache.resolvedLinks[file2.path];
					}
					debugLog(
						"getAttachmentsInVaultByLinks - resolvedLinks:",
						resolvedLinks,
					);
				}
			}
			...
			if (
				...
				if (rlinks) {
					...
					resolvedLinks[file.path] =
						this.app.metadataCache.resolvedLinks[oldPath];
				}
				debugLog(
					"getAttachmentsInVaultByLinks - resolvedLinks:",
					resolvedLinks,
		...
		debugLog(
			"getAttachmentsInVaultByLinks - allFiles:",
			allFiles.length,
			allFiles,
		);
		if (resolvedLinks) {
					...
					for (const k of Object.keys(frontmatter)) {
						if (typeof frontmatter[k] === "string") {
							...
							if (formatMatch && formatMatch[1]) {
								...
								const file2 = this.app.metadataCache.getFirstLinkpathDest(
									fileName,
									obsFile.path,
								);
				...
				if (canvasData.nodes && canvasData.nodes.length > 0) {
					for (const node of canvasData.nodes) {
						if (node.type === "file") {
							...
							const linkMatches = await getAllLinkMatchesInFile(
								obsFile,
								app,
								node.text,
							);
...
var ConfirmModal = class extends import_obsidian9.Modal {
	...
	onOpen() {
		...
		contentEl.createSpan("", (el) => {
			el.innerText =
				"This operation is irreversible and experimental. Please backup your vault first!";
		});
...
var CreateHandler = class {
	...
	processAttach(attach, source) {
		...
		if (
			extSetting === void 0 &&
			!isImage(attach.extension) &&
			!isPastedImage(attach)
		) {
		...
		const attachPath = metadata.getAttachmentPath(
			setting,
			this.settings.dateFormat,
		);
		metadata
			...
			.then((attachName) => {
				...
				this.app.vault.adapter
					...
					.finally(() => {
						const attachPathFolder =
							this.app.vault.getAbstractFileByPath(attachPath);
						deduplicateNewName(attachName, attachPathFolder).then(
							({ name }) => {
								debugLog(
									"processAttach - new path of file:",
									path.join(attachPath, name),
								);
	...
	renameCreateFile(attach, attachPath, attachName, source) {
		const dst = (0, import_obsidian10.normalizePath)(
			path.join(attachPath, attachName),
		);
...
var AttachmentManagementPlugin = class extends import_obsidian11.Plugin {
	...
	async onload() {
		...
		console.log(
			`Plugin loading: ${this.manifest.name} v.${this.manifest.version}`,
		);
		this.app.workspace.onLayoutReady(() => {
			...
			this.registerEvent(
				...
				this.app.vault.on("rename", async (file, oldPath) => {
					...
					const { setting } = getRenameOverrideSetting(
						this.settings,
						file,
						oldPath,
					);
					...
					if (
						setting.type === "FOLDER" /* FOLDER */ ||
						setting.type === "FILE" /* FILE */
					) {
					...
					if (!this.settings.autoRenameAttachment) {
						debugLog(
							"rename - auto rename not enabled:",
					...
					if (file instanceof import_obsidian11.TFile) {
						...
						if (isAttachment(this.settings, file)) {
							debugLog(
								"rename - not processing rename on attachment:",
								file.path,
							);
							return;
						}
						await new ArrangeHandler(
							this.settings,
							this.app,
							this,
						).rearrangeAttachment(2 /* FILE */, file, oldPath);
						const oldMetadata = getMetadata(oldPath);
						const oldAttachPath = oldMetadata.getAttachmentPath(
							setting,
							this.settings.dateFormat,
						);
						this.app.vault.adapter
							...
							.then((exists) => {
								if (exists) {
									checkEmptyFolder(this.app.vault.adapter, oldAttachPath).then(
										(empty) => {
											if (empty) {
			...
			this.registerEvent(
				this.app.vault.on("delete", async (file) => {
					...
					if (
						...
						debugLog(
							"delete - exclude path or the file is an attachment:",
							file.path,
						);
						return;
					}
					if (deleteOverrideSetting(this.settings, file)) {
						...
						new import_obsidian11.Notice(
							"Removed override setting of " + file.path,
						);
					}
					if (file instanceof import_obsidian11.TFile) {
	...
	initCommands() {
				...
				if (file) {
					...
					if (!checking) {
						...
						this.saveSettings().finally(() => {
							new import_obsidian11.Notice(
								`Reset attachment setting of ${file.path}`,
							);
						});
					}
					return true;
		...
		this.addCommand({
			...
			callback: async () => {
				const attachments = await new ArrangeHandler(
					this.settings,
					this.app,
					this,
				).getAttachmentsInVault(this.settings, 1 /* LINKS */);
				const storages = [];
				for (const attachs of Object.values(attachments)) {
					for (const attach of attachs) {
						...
						if (
							linkFile !== null &&
							linkFile instanceof import_obsidian11.TFile
						) {
							md5sum(this.app.vault.adapter, linkFile).then((md5) => {
								const ret = this.settings.originalNameStorage.find(
									(data) => data.md5 === md5,
								);
								if (ret) {
									storages
										.filter((n) => n.md5 == md5)
										.forEach((n) => storages.remove(n));
									storages.push(ret);
								}
							});
						}
